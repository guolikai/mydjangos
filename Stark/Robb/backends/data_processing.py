...
class DataHandler(object):
    ...
    def data_point_validation(self,host_obj,service_obj):
        ...
        if latest_data_point: #data list is not empty,
            ...
            if time.time() - last_report_time > monitor_interval: #超过监控间隔但数据还没汇报过来,some thing wrong with client
                no_data_secs =  time.time() - last_report_time
                print("\033[41;1mSome thing must be wrong with client [%s] , because haven't receive data of service [%s]\
    ...
    def load_service_data_and_calulating(self,host_obj,trigger_obj,redis_obj):
        ...
        print(u"###表达式名称[%s],结果:[%s]###" % (trigger_obj.name,expression_res_string))

        if expression_res_string:
            trigger_res = eval(expression_res_string)
            #print("whole_trigger_result:", trigger_res )
            if trigger_res:#终于走到这一步,该触发报警了
                print(u"触发报警trigger alert:告警级别是%s,结果:[%s]" % (trigger_obj.severity,trigger_res))
    ...
    def trigger_notifier(self,host_obj,trigger_id, positive_expressions,msg,redis_obj=None):
        '''
        ...
        print(u'主机 %s 触发告警:[trigger_id]:%s 表达式:%s REDIS对象:%s' % (host_obj,trigger_id, positive_expressions,redis_obj) )
        ...
        #先把之前的Trigger加载回来，获取上次报警的时间，已统计故障持续时间
        ...
        if old_trigger_data:
            ...
            msg_dic['start_time'] = trigger_starttime
            msg_dic['duration'] = round(time.time() - trigger_starttime)
        #print("msg_dic",type(msg_dic),msg_dic)

        #同时在Redis中记录这个Trigger，前端页面展示时要统计Trigger个数
        #一个trigger纪录5分钟后会自动清除, 为了在前端统计trigger个数用的
...
class ExpressionProcess(object):
    ...
    def __init__(self,main_ins,host_obj,expression_obj,specified_item=None):
        ...
        print("\033[31;1m--->Redis数据库存储的Key: %s \033[0m" % self.service_redis_key)
    ...
    def load_data_from_redis(self):
        ...
        print(u"[%s]秒内需要大概从数据库获取个数: %s " % (time_in_sec,approximate_data_points))
        try:
            data_range_raw = self.main_ins.redis.lrange(self.service_redis_key,-approximate_data_points,-1)
        except redis.exceptions.ResponseError as e:
            #print("\033[31;1m--->Not enought data for this range...\033[0m")
        ...
        print(u"Redis获取的数据:",data_range)
        return data_range

    def process(self):
        data = self.load_data_from_redis() #已经按照用户的配置把数据 从redis里取出来了;比如最近5分钟或10分钟的数据
        data_calc_func = getattr(self,'get_%s' % self.expression_obj.data_calc_func)
        single_expression_calc_res = data_calc_func(data)
        print(u"\033[41;1mResult of single_expression_calc_res:[返回结果,返回值,Key值]\033[0m",single_expression_calc_res) #已经触发的告警相关信息
    ...
    def get_avg(self,data_set):
        ...
        if clean_data_list:
            ...
            print("Clean_Data List:", clean_data_list)
            return [self.judge(avg_res), avg_res,None]  #表达式值与阈值进行比较；
            ...
            for k,v in clean_data_dic.items():
                ...
                print("\033[46;1m[%s]-->avg_result: %s\033[0m" % (k,avg_res))
                ...
                else:#监控这个服务 的所有项, 比如一台机器的多个网卡, 任意一个超过了阈值,都算是有问题的
                    calc_res = self.judge(avg_res)
                    if calc_res:
                        return [calc_res,avg_res,k]
                print(u'表达式告警阈值specified monitor key:',self.expression_obj.specified_index_key)  #监控表达式告警阈值;
                print('Clean_Data Dic:监控项:[%s],数据长度:[%s],数据:[%s]'% (k,len(clean_v_list),clean_v_list))
