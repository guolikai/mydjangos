...
class ClientHandler(object):
    ...
    def fetch_configs(self):
        try:
            ...
            template_list = list(host_obj.templates.select_related())   #主机直接关联的模板

            for host_group in host_obj.host_groups.select_related():
                template_list.extend(host_group.templates.select_related()) #主机组直接关联的模板
            #print(u'-->客户端所属模板:',template_list)

            for template in template_list:
                for service in  template.services.select_related():
                    print(u'客户端需监控的服务插件:',service)
...
def get_host_triggers(host_obj):
    print("\033[42;1m----Triggers Operation----\033[0m")
