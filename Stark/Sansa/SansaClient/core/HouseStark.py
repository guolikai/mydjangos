...
class ArgvHandler(object):
    ...
    def collect_data(self):
        ...
        asset_data = obj.collect()  #收集客户端数据
        print(asset_data)
    ...
    def __submit_data(self,action_type,data,method):
        ...
        if action_type in settings.Params['urls']:
            ...
            print('---> Connecting Server [%s], it may take a minute' % url)
            if method == "get":
                args = ""
                ...
                try:
                    #data_encode = urllib.urlencode(data)   #python2
                    #data_encode = UrllibParse.urlencode(data).encode(encoding='UTF8')
                    data_encode = urllib.parse.urlencode(data).encode()
                    #print('data',type(data_encode))
    ...
    def __update_asset_id(self,new_asset_id):
        ...
        f.write(new_asset_id.encode())
        f.close()

    def report_asset(self):
        obj = info_collection.InfoCollection()
        asset_data = obj.collect()              #Collection Server Info Data
        ...
        print(u'---> Client_Collect_Data:',asset_data)
        ...
        if type(log) is dict:
            ...
            if "warning" in log:
                for msg in log["warning"]:
                    ...
                    #print(type(log_format.encode()))
                    f.write(log_format.encode())    #字符bytes可以写入文件；
        f.close()