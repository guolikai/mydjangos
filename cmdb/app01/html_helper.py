...
def Pager(page,pages_all_count):
    ...
    page_html = []  
    first_html = "<a href='/app01/host_list/%d'>首页 </a>" % (1)
    page_html.append(first_html)
    if page <=1:
        prev_html = "<a href='#'>上一页 </a>"
    else:    
        prev_html = "<a href='/app01/host_list/%d'>上一页 </a>" % (page-1)
    page_html.append(prev_html)
    for i in range(pages_all_count):
        if page == i+1:
            a_html = "<a class='selected' href='/app01/host_list/%d'>%d </a>" % (i+1,i+1)
        else:
            a_html = "<a href='/app01/host_list/%d'>%d </a>" % (i+1,i+1)
        page_html.append(a_html)
        
    if page+1 > pages_all_count:
        next_html =  "<a href='#'>下一页 </a>"
    else:     
        next_html = "<a href='/app01/host_list/%d'>下一页 </a>" % (page+1)
    page_html.append(next_html)
       
    end_html =  "<a href='/app01/host_list/%d'>尾页 </a>" % (pages_all_count)
