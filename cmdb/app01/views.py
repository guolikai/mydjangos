...
def Index(request,*args,**kwargs):
    ...
    #return render(request,'app01/index.html',{'username':request.session.get('is_login')['user']})
    return render(request,'app01/base.html',{'username':user_dict['user']})
...
def Login(request):
    ...
    return render(request,"app01/login.html",ret)
...
def Register(request):
    ...
    return render(request,"app01/register.html",ret)
...
def host_add(request):
    ...
    return render(request,"app01/host_add.html",ret)
'''def List(request):
    ret = {'data':None}
    data = models.Asset.objects.all()
    ret['data'] = data
    return render(request,"app01/host_list.html",ret)
    '''
@outer
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户组删除成功"
                return redirect("/app01/host_del")
        else:
            ret['status'] = "用户组名不能为空" 
    data = models.Asset.objects.all()
    ret['data'] = data    
    return render(request,"app01/host_del.html",ret)
        ...
        if is_empty:
            ...
            if count==0:
                ...
                if str(selectval) == 'Ip':  
                    ...
                    ret['status']="用户邮箱修改成功"
                return redirect("/app01/host_mod")
        else:
            ret['status'] = "用户修改信息不能为空"  
    return render(request,"app01/host_mod.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==1:
                ...
                ret['status']="用户类型添加成功"
                return redirect("/app01/type_add")
        else:
            ret['status'] = "Usertype不能为空" 
    data = models.UserType.objects.all()
    ret['data'] = data    
    return render(request,"app01/type_add.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户类型删除成功"
                return redirect("/app01/type_del")
        else:
            ret['status'] = "Usertype不能为空" 
    data = models.UserType.objects.all()
    ret['data'] = data    
    return render(request,"app01/type_del.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户类型修改成功"
                return redirect("/app01/type_mod")
        else:
            ret['status'] = "Usertype不能为空" 
    data = models.UserType.objects.all()
    ret['data'] = data    
    return render(request,"app01/type_mod.html",ret)
...
def type_get(request):
    ...
    data = models.UserType.objects.all()
    ret['data'] = data    
    return render(request,"app01/type_get.html",ret)
...
def user_add(request):
    ...
    if request.method == 'POST':
        ...
        usertype_id = request.POST.get('usertype',None)
        is_empty = all([username,password,email])
        #print(username,password,email,usertype_id)
        if is_empty:
            usertypeobj = models.UserType.objects.get(id=usertype_id)
            models.UserInfo.objects.create(username=username,
                                        password=password,
                                        ...
                                        user_type=usertypeobj)
            ret['status']="注册成功"
            return redirect("/app01/user_add")
        else:
            ret['status'] = "Username、Password、Email不能为空"     
    return render(request,"app01/user_add.html",ret)

@outer
def user_get(request):
    user_dict = request.session.get('is_login',None)
    ret = {'status':'','data':'','username':user_dict['user']}
    #data = models.UserInfo.objects.filter(username=user_dict['user']) #过滤自己用户信息
    data = models.UserInfo.objects.all() #查找所有用户信息
    ret['data'] = data    
    return render(request,"app01/user_get.html",ret)
        ...
        if is_empty:
            ...
            if count==0:
                ...
                if str(selectval) == 'password':  
                    ...
                    ret['status']="用户邮箱修改成功"
                #return redirect("/app01/user_mod")
        else:
            ret['status'] = "用户修改信息不能为空"  
    return render(request,"app01/user_mod.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户名删除成功"
                return redirect("/app01/user_del")
        else:
            ret['status'] = "用户名不能为空"  
    return render(request,"app01/user_del.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==1:
                ...
                ret['status']="用户组添加成功"
                return redirect("/app01/group_add")
        else:
            ret['status'] = "用户组不能为空"
    return render(request,"app01/group_add.html",ret)
...
def group_get(request):
    ...
    ret['data'] = data    
    return render(request,"app01/group_get.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户组删除成功"
                return redirect("/app01/group_del")
        else:
            ret['status'] = "用户组名不能为空" 
    data = models.UserGroup.objects.all()
    ret['data'] = data    
    return render(request,"app01/group_del.html",ret)
    ...
    if request.method == 'POST':
        ...
        if is_empty:
            ...
            if count==0:
                ...
                ret['status']="用户组修改成功"
                return redirect("/app01/group_mod")
        else:
            ret['status'] = "用户组名不能为空" 
    data = models.UserGroup.objects.all()
    ret['data'] = data    
    return render(request,"app01/group_mod.html",ret)
